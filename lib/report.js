...
export function terminal(s, roster) {
  ...
  lines.push('');
  lines.push(`  ${s.calls} Jev calls, ${s.tokens.input.toLocaleString()} input tokens, about $${s.cost.toFixed(3)}, ${s.avgMs} ms per judged prompt on average.`);
  return lines.join('\n');
}
...
<td><small>${esc((c.timestamp ?? '').slice(0, 16))}<br>${esc(c.project.replace(/^-/, '').replace(/-/g, '/').slice(-40))}</small></td>
...
td.p{max-width:520px}.t{white-space:pre-wrap;word-break:break-word}.r{color:var(--muted);font-size:12.5px;margin-top:4px;white-space:pre-wrap}
...
<table><thead><tr><th>Result</th><th>Prompt</th><th>Jev pick</th><th>Turn loaded</th><th>When, where</th><th>Label</th></tr></thead>
