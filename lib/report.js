...
export function summarize(cases) {
  ...
  const bySkill = {};
  const modSessions = new Set(cases.filter(c => c.suggested).map(c => c.session));
  const obedience = { suggested: 0, followed: 0 };
  const split = { withMod: { need: 0, miss: 0 }, without: { need: 0, miss: 0 } };
  const NEED = new Set(['miss', 'hit', 'already-loaded', 'disagree']);
  let rostered = 0;
  for (const c of cases) {
    ...
    if (c.suggested) { obedience.suggested++; if (c.followed) obedience.followed++; }
    if (c.rosterSize) rostered++;
    const side = modSessions.has(c.session) ? split.withMod : split.without;
    if (NEED.has(c.category)) { side.need++; if (c.category === 'miss') side.miss++; }
  }
...
export function terminal(s, roster) {
  ...
  if (s.obedience.suggested) {
    lines.push('');
    lines.push(`  The mod attached a suggestion on ${s.obedience.suggested} turns in ${s.modSessions} sessions; the agent loaded that skill on ${s.obedience.followed} (${pct(s.obedience.followed, s.obedience.suggested)}).`);
    lines.push(`  Miss rate with the mod active: ${s.split.withMod.miss}/${s.split.withMod.need} (${pct(s.split.withMod.miss, s.split.withMod.need)}). Without: ${s.split.without.miss}/${s.split.without.need} (${pct(s.split.without.miss, s.split.without.need)}).`);
  }
  lines.push('');
  lines.push(`  ${s.rostered} of ${s.total} prompts were judged against the skill list their own session showed the model; the rest against what is installed now.`);
...
<td>${c.suggested ? `${esc(c.suggested)}<br><small class="${c.followed ? 'ok' : 'no'}">${c.followed ? 'followed' : 'ignored'}</small>` : '<small>off</small>'}${c.rosterSize ? `<br><small>${c.rosterSize} listed</small>` : ''}</td>
...
small.ok{color:var(--ok)}small.no{color:var(--bad)}
...
${s.obedience.suggested ? `<p><b>The mod, live:</b> it attached a suggestion on ${s.obedience.suggested} turns across ${s.modSessions} sessions and the agent loaded that skill on ${s.obedience.followed} (${pct(s.obedience.followed, s.obedience.suggested)}). Miss rate in those sessions ${pct(s.split.withMod.miss, s.split.withMod.need)} (${s.split.withMod.miss} of ${s.split.withMod.need}) against ${pct(s.split.without.miss, s.split.without.need)} (${s.split.without.miss} of ${s.split.without.need}) elsewhere.</p>` : ''}
<p>${s.rostered} of ${s.total} prompts were judged against the skill list their own session showed the model; the rest against what is installed now.</p>
...
<div style="overflow-x:auto"><table><thead><tr><th>Result</th><th>Prompt</th><th>Jev pick</th><th>Turn loaded</th><th>Mod said</th><th>When, where</th><th>Label</th></tr></thead>
