...
import { parseListing, sameSkill } from './roster.js';

// What the mod attaches; its trace in a transcript is the proof it ran.
export const RELEVANCE_RE = /<skill_relevance>[\s\S]*?Relevant to this request: ([\w:.-]+)\.[\s\S]*?<\/skill_relevance>/;

const stripReminders = t => t.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g, '').replace(RELEVANCE_RE, '').trim();
...
function relevanceIn(rec) {
  const a = rec.attachment;
  const parts = [];
  if (a) {
    if (typeof a.content === 'string') parts.push(a.content);
    else if (Array.isArray(a.content)) parts.push(...a.content.map(String));
  }
  const t = textOf(rec.message?.content);
  if (t) parts.push(t);
  for (const p of parts) {
    const m = RELEVANCE_RE.exec(p);
    if (m) return m[1];
  }
  return null;
}
 ...
 * `roster` is the skill list Claude Code showed the model at that point, when
 * the transcript recorded one (null otherwise); `suggested` is the skill the
 * mod attached to this prompt, if it ran.
 */
export async function* turns(session, { isSkill = () => true } = {}) {
  let cur = null;
  ...
  for await (const rec of records(session.path)) {
    if (rec.type === 'attachment') {
      ...
      if (a.type === 'skill_listing') {
        const listed = parseListing(a.content);
        // A fresh array per change, so turns sharing a roster share one reference.
        roster = a.isInitial || !roster ? listed : [...roster, ...listed.filter(s => !roster.some(r => r.name === s.name))];
        if (cur && !cur.roster) cur.roster = roster;
      } else if (cur) {
        const s = relevanceIn(rec);
        if (s) cur.suggested = s;
      }
      continue;
    }
    if (rec.type === 'user' && !rec.isSidechain) {
      ...
      if (cmd) {
        // /compact or /model is a built-in command, not a skill load.
        if (known(cmd)) { if (cur) cur.loadedNow.push(cmd); else loadedBefore.add(cmd); }
        continue;
      }
      if (isHumanPrompt(rec)) {
        const raw = textOf(rec.message?.content);
        ...
        cur = {
          ...
          loadedNow: [],
          roster,
          suggested: relevanceIn(rec),
        };
        continue;
      }
      if (cur) {
        cur.loadedNow.push(...skillLoadsIn(rec));
        const s = relevanceIn(rec);
        if (s && !cur.suggested) cur.suggested = s;
      }
      continue;
    }
