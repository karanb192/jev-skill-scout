...
const MARKET_ROOT = ['.claude', 'plugins', 'marketplaces'];
...
export async function readRoster(fs, { home, cwd, bodyChars = 700 }) {
  ...
  // A marketplace checkout can carry skills of its own, listed alongside plugin skills.
  const markets = [home, ...MARKET_ROOT].join('/');
  for (const market of await dirs(markets)) {
    const skillsDir = `${markets}/${market.name}/skills`;
    for (const s of await dirs(skillsDir)) await add(`${skillsDir}/${s.name}`, s.name, `marketplace ${market.name}`);
  }
  return [...out.values()];
}
