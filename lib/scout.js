...
// A Choice takes at most 255 options; rosters past this are ranked in chunks.
...
async function askOnce(fetchImpl, key, body, timeoutMs) {
  ...
  let timer = null;
  // Two guards: the abort for transports that honour it, the race for those that do not.
  const deadline = new Promise((_, reject) => { timer = setTimeout(() => { controller?.abort(); reject(new Error('aborted: timeout')); }, timeoutMs); });
  const work = (async () => {
...
/** Re-applies the thresholds to a stored judgment, so a rerun with new ones costs nothing. */
export function decide(res, options = {}) {
  const o = { ...DEFAULTS, ...options };
  if (!res || !res.rank) return null;
  if (res.rank.gate < o.gateThreshold) return null;
  const v = res.verify;
  if (!v || !v.winner || v.winner === NONE) return null;
  return (v.fits[v.winner] ?? 0) >= o.fitsThreshold ? v.winner : null;
}
...
export async function suggest({ fetchImpl, key, roster, request, recent = '', options = {} }) {
  const o = { ...DEFAULTS, ...options };
  const t0 = Date.now();
  if (!roster.length) return { suggestion: null, rank: { ranked: [[NONE, 1]], gate: 0, gates: {}, confidence: null }, verify: null, usage: { input: 0, output: 0, calls: 0 }, ms: 0, stage: 'empty' };
  const chunks = [];
  for (let i = 0; i < roster.length; i += CHUNK) chunks.push(roster.slice(i, i + CHUNK));
  const replies = await Promise.all(chunks.map(c => ask(fetchImpl, key, buildRank(c, request, recent, o), o.timeoutMs, o.retries ?? 0)));
  const usage = { input: 0, output: 0, calls: replies.length };
  for (const r of replies) { usage.input += r.usage?.input_tokens ?? 0; usage.output += r.usage?.output_tokens ?? 0; }
  const rank = mergeRanks(replies.map(r => readRank(r.answers)), o.shortlist);
  ...
  out.stage = 'verify';
  out.ms = Date.now() - t0;
  out.suggestion = decide(out, o);
  return out;
}

// Probabilities from different chunks are not comparable, so each chunk keeps
// only its own top few and the verify stage settles it with real excerpts.
function mergeRanks(ranks, shortlist) {
  if (ranks.length === 1) return ranks[0];
  const ranked = [];
  let gate = 0;
  for (const r of ranks) {
    ranked.push(...r.ranked.filter(([n]) => n !== NONE).slice(0, shortlist));
    gate += r.gate;
  }
  ranked.sort((a, b) => b[1] - a[1]);
  ranked.push([NONE, Math.max(...ranks.map(r => r.ranked.find(([n]) => n === NONE)?.[1] ?? 0))]);
  return { ranked, gate: gate / ranks.length, gates: ranks[0].gates, confidence: null };
}
